...
@author: Jonathan Young with minor edits from Eitan Halper-Stromberg
...
def get_path(win, name, label):
    ...
    path = tkfd.askopenfilename(parent=win, title="Select file for '"+name+"'",
                                ...
                                initialdir='\\pathology21\Molecular\ChimerAnalyzer BME files')#'C:')
...
def evaluate_file(file_path, output_dict, control, ht_dict,pre_or_don):
                        ...
                        for s, h in v.items():
                            ...
                            if (s-0.3) <= size <= (s+0.3) and h > 6400:
                                ...
                                if peak == False:
                                    ...
                                    if tkmb.askyesno(message='\n\nDo you want to remove the peak for this locus?\n'+\
                                        all_loci[np.where([i == key for i in simple_loci])[0][0]]+'\n'+\
                                        'color key: '+key+' size: '+str(size)+' height: '+str(height)+' sample: '+pre_or_don, icon='question', title='Remove Peak?'):
                                        if size in output_dict[key]:
                                            output_dict[key].remove(size)
...
# LOGIC 1    
evaluate_file(path_dict['pre'], pre_dict, pre_control, pre_ht,'pre')
evaluate_file(path_dict['don'], don_dict, don_control, don_ht,'don')
...
fout = None
while fout == None:
    fout = tkfd.asksaveasfile(parent=root, mode='w',
                          title='Select Where to Save "Comp" Results',
                          defaultextension='.csv',
                          initialfile='comp_results'+str(datetime.date.today()),
                          filetypes=[('CSV file', '.csv')],
                          initialdir='C:') ## change this to your default download directory

w_comp = csv.writer(fout,lineterminator='\n')
